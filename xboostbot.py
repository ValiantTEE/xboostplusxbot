...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    msg = (
        ...
        "💎 Real. Permanent. Powerful.\n"
        "💎 Verified users that add instant trust\n"
        "💎 Designed for creators, KOLs & businesses\n"
        "💎 Take your X presence to the next level\n\n"
        "Please note:\n\n"
        "Pinned tweets won't be boosted. Pin them after they're boosted.\n\n"
        "Edited tweets won't be boosted. To fix, delete and post again.\n\n"
        "👉 Click \"Subscribe\" to start boosting your tweets and growing your X account today!\n\n"
        "Perfect for: KOLs, Crypto Projects, Businesses, and Personal Accounts.\n\n"
...
async def callback_router(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Predefine the full start/main message so all branches can reuse it
    start_msg = (
        "Welcome to XBoostPlus! 🚀\n"
        "Your tweets get boosted automatically right after you post — no extra work needed!\n\n"
        "✅ Get Real followers — no bots, no fakes\n"
    ...
    if data == CB_MAIN_MENU:
        context.user_data["nav_stack"] = []
        push_nav(context, "main_menu")
        await query.edit_message_text(start_msg, reply_markup=main_menu_keyboard())
        return

    # --- Back navigation ---
    if data == CB_BACK:
        # Pop current page, then peek at previous
        _ = pop_nav(context)
        prev = current_nav(context)

        # If no previous page, go to main menu
        if not prev:
            context.user_data["nav_stack"] = []
            push_nav(context, "main_menu")
            await query.edit_message_text(start_msg, reply_markup=main_menu_keyboard())
        ...
        if page == "select_account_for_service":
            ...
            return
        elif page == "service_packages":
            await show_service_packages(query, context, payload["service_key"], push_stack=False)
            return
        elif page == "orders_accounts":
            await show_accounts_for_orders(query, context, push_stack=False)
            return
        elif page == "adding_account":
            add_msg = (
                "➕ Manage Your X Accounts Easily\n\n"
                "Add your X (Twitter) account below — you can connect multiple accounts.\n\n"
                "Remove any account when you're not using it.\n\n"
                "Go to Subscribe to activate a boost on an account of your choice.\n\n"
                "Please send your X handle as username only (no @). Example: user123"
            )
            await query.edit_message_text(add_msg, reply_markup=back_and_menu_keyboard())
            return
        elif page in ("package_selected", "payment_page", "awaiting_proof"):
            # Go back to the service package page
            pkg_payload = payload or context.user_data.get("last_package")
            if pkg_payload:
                service_key = pkg_payload["service_key"]
                await show_service_packages(query, context, service_key, push_stack=False)
            else:
    ...
    # --- User selected account for a service ---
    ...
    if data == "paid":
        ...
        await query.edit_message_text(
            "Please upload the transaction hash as text or a screenshot as proof of payment.",
            reply_markup=back_and_menu_keyboard()
        )
        return
    ...
    # --- View orders for a specific account ---
...
async def handle_tx_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    last_package = context.user_data.get("last_package")
    if not last_package:
        # If no package in flow, fallback to account-add handler
...
async def handle_photo(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await context.bot.send_photo(chat_id=ADMIN_ID, photo=file_id, caption=f"Screenshot from @{sender_username}")
    except Exception:
...
async def admin_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    async with aiosqlite.connect(DB_PATH) as db:
        cur = await db.execute("""
            SELECT o.id, u.telegram_id, u.username, o.package, o.price, o.chain, o.status, o.created_at
            FROM orders o
    ...
    text = "Pending orders:\n\n"
    for r in rows[:50]:
        oid, telegram_id, username, package, price, chain, status, created_at = r
        text += f"ID: {oid} | User: @{username} ({telegram_id}) | {package} | ${price} | {chain} | {created_at}\n"
...
async def admin_complete(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if row:
        user_rowid = row[0]
        # Look up telegram id from users table
        async with aiosqlite.connect(DB_PATH) as db:
            cur = await db.execute("SELECT telegram_id FROM users WHERE id = ?", (user_rowid,))
            urow = await cur.fetchone()
        if urow:
            telegram_id = urow[0]
            try:
                await context.bot.send_message(chat_id=telegram_id, text=f"✅ Your order {oid} has been marked as completed by admin.")
            except Exception:
                logger.exception("Failed to notify user about completion")
...
async def main():
    ...
    logging.info("Starting XBoostPlus bot...")
    await app.bot.delete_webhook(drop_pending_updates=True)
    
    # instead of asyncio.run(app.run_polling()), just do
    app.run_polling()

if __name__ == "__main__":
    import logging
    logging.basicConfig(level=logging.INFO)
    main()  # call main() directly, it will start polling